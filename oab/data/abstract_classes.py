...
# edit new imports START#
from ruamel.yaml import YAML
import yaml
# edit new imports END#
	

from oab.data.utils import _append_to_yaml, _make_yaml, _read_from_yaml, _get_dataset_dict
...
class AnomalyDataset(ABC):
    ...
    def sample_from_yaml(self, filepath: str = None,
        ...
        #EDIT FOR CHOOSING DYNAMIC SAMPLING TYPE FROM FILE START#
        yaml=YAML(typ='rt')
        yaml_data = yaml.load(Path("./") / filepath)
        x=yaml_data["sampling"] 
        type=[i for i in x.keys()][0]
        #print(type)
        
        #EDIT FOR CHOOSING DYNAMIC SAMPLING TYPE FROM FILE END#
